...
public class DatabaseEventShould {
    ...
    public void correctlyTellIfWeAreContainedInTheEvent() {
        ...
        userWithOnlyEmail.email = Account.shared.getEmail().get();
    ...
    public void correctlyConvertToEvent() {
        ...
        String name = "GroupUp Event";
        String description = "This is going to be fun.";
        String datetimeStart = LocalDateTime.now().toString();
        String datetimeEnd = LocalDateTime.now().plusDays(1).toString();
        String uuid = "myEventUuidIsReallyComplex";
        ...
        assertEquals(uuid, event.getUUID());
        assertEquals(uuid, event.getUUID());
        assertEquals(members.size(), event.getEventMembers().size());
        assertEquals(pointsOfInterest.size(), event.getPointsOfInterest().size());
