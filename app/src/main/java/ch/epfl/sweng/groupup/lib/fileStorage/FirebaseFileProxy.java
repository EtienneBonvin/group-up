...
public class FirebaseFileProxy implements FileProxy, Watchee {
    ...
    public FirebaseFileProxy(Event event) {
        ...
        ticker = new TimeBomb(event.getEventMembers()
                                   .size());
        ...
        for (Member member : event.getEventMembers()) {
            memberCounter.put(member.getUUID()
                                    .getOrElse("Default ID"), new Counter());
        }
                                   ...
                                   fileRef.getBytes(Long.MAX_VALUE)
                                          .addOnSuccessListener(new OnSuccessListener<byte[]>() {
                                              ...
                                              public void onSuccess(byte[] bytes) {
                                                  recoveredImages.add(new CompressedBitmap(bytes));
                                                  memberCount.increment();
                                                  notifyAllWatchers();
                                                  ticker.tick(true);
                                              }
    ...
    private void removeOneFile(final int index) {
        StorageReference imageRef = storageRef.child(event.getUUID() + "/" + Account.shared.getUUID() + "/" + index);
